...
import { cn, getApiError, getAuthorName } from "@/lib/utils";
...
const Borrow: React.FC<IBorrowBookModalProps> = ({
  ...
  const schema = useMemo(
    () => borrowBookSchema(bookData?.availableCopies || 0, validationMessages),
  ...
  const onSubmit = async (values: BorrowFormData) => {
    ...
    try {
      ...
      toast.error(getApiError(err, "Failed to borrow book"));
    }
  };
